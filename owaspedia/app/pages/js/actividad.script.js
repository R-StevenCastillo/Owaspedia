...
// Function to load quiz data from Data.json file
const loadQuizData = async () => {
  const res = await fetch("../json/actividadData.json");
...
for (let i = 0; i < 4; i++) {
  document.getElementById(`btn${i}`).addEventListener("click", (event) => {
    ...
    if (
      ...
      document.getElementById("message").innerText = "Respuesta Correcta!";
    } else {
      selectedOption.className = "option-btn wrong";
      document.getElementById("message").innerText = "Respuesta Erronea!";
    }
    for (let j = 0; j < 4; j++) {
